...
class BTPlusPlus extends Mutator config(BTPlusPlusPublic);
...
function PreBeginPlay()
{
	...
	if(!Initialized)
	{
		...
		if(Role == ROLE_Authority)
		{
			...
			log("+-----------------", tag);
			log("| BTPlusPlus UTBT Public Edition", tag);
		...
		initPlayerStarts();
		// Spawn FlagReplacer
		FR = Spawn(class'UTBT_FlagReplacer');
	}
	Super.PreBeginPlay();
}
...
function SetBestTime(int Time, int TimeStamp, int i, string ctf)
{
	...
	if(Time > MapBestTime && !PI[i].RI.bBoosted)
	{
		MapBestTime = Time;
...
function MutatorTakeDamage( out int ActualDamage, Pawn Victim, Pawn InstigatedBy, out Vector HitLocation, out Vector Momentum, name DamageType)
{
	...
	if(InstigatedBy != None && PlayerPawn(InstigatedBy) != None && Victim != InstigatedBy && PlayerPawn(Victim) != None)
	{
		...
		if(PI[VictimID].Config.bAntiBoost)
		{
			Momentum = Vect(0,0,0);
			ActualDamage = 0;
		}
...
//xxx		//Other.SetCollision(Other.Default.bCollideActors);
...
function bool MutatorTeamMessage(Actor Sender, Pawn Receiver, PlayerReplicationInfo PRI, coerce string S, name Type, optional bool bBeep)
{
	...
	if(Sender == Receiver)
	{
		...
		else if(Sender.IsA('TournamentPlayer'))
		{
			...
			{
				if(PlayerPawn(Sender).PlayerReplicationInfo.Team == 0)
					PlayerPawn(Sender).ClientMessage("You're on Red team already.");
				else
					PlayerPawn(Sender).ChangeTeam(0);
			}
			else if(S ~= "!BLUE" || S ~= "!B")
			{
				if(PlayerPawn(Sender).PlayerReplicationInfo.Team == 1)
					PlayerPawn(Sender).ClientMessage("You're on Blue team already.");
				else
					PlayerPawn(Sender).ChangeTeam(1);
			}
			else if(S ~= "!SPEC" || S ~= "!S")
			{
		...
		else if(BTSpectator(Sender) != None)
		{
			if(S ~= "!PLAY" || S ~= "!P")
			{
				PlayerPawn(Sender).PreClientTravel();
...
function bool MutatorBroadcastMessage(Actor Sender, Pawn Receiver, out coerce string Msg, optional bool bBeep, out optional name Type)
{
	if(Sender == Receiver)
	{
		...
		{
			if(Right(Msg, 4) ~= "!PLAY" || Right(Msg, 2) ~= "!P")
			{
				PlayerPawn(Sender).PreClientTravel();
				PlayerPawn(Sender).ClientTravel("?OverrideClass=", TRAVEL_Relative, false);
			}
...
function OpenSettingsWindow(PlayerPawn Sender)
{
	...
	if(BTSpectator(Sender) != None)
		...
		BTWRI.BTCS = PI[FindPlayer(Sender)].BTCS;
}
//====================================
