...
{
	local BTClientSettings C;

	foreach AllActors(class'BTClientSettings', C)
	{
		if(C.Owner == PlayerOwner)
		{
			BTEC = C;
...
simulated function PostRender( canvas Canvas )
{
	...
	else
	{
		...
		if(BTEC.TransparentSelf)
		{
			if(PlayerOwner.Style != STY_Translucent)
				PlayerOwner.Style = STY_Translucent;
			if(PlayerOwner.Weapon != None && PlayerOwner.Weapon.Style != STY_Translucent)
				PlayerOwner.Weapon.Style = STY_Translucent;
		}
	}
	...
	if ( (!PlayerOwner.bBehindView || (PlayerOwner.bBehindView && PlayerOwner.ViewTarget == None && !PlayerOwner.IsInState('GameEnded') && BTEC != None && BTEC.BehindviewCrosshair))
	...
	if( !bHideHUD )
	{
		if ( !PawnOwner.PlayerReplicationInfo.bIsSpectator )
		{
			...
			if(BTEC != None && (BTEC.ShowSpeedoMeterHorizontal || BTEC.ShowSpeedoMeterVertical))
...
simulated function DrawHUDTimes(Canvas Canvas, int Minutes, int Seconds)
{
	...
	if(BTEC != None && BTEC.CustomTimer)
	{
		...
		Canvas.CurX = (Canvas.ClipX / 2 - 86.5 * TimerScale) + BTEC.TimerLocationX;
		Canvas.CurY = 4 + BTEC.TimerLocationY;
	}
	else
	{
		TimerScale = Scale;
	...
	else //	show actual timer
	{
		if(RI != None && RI.bNeedsRespawn)	//Captime/reset on death in grey
			...
			Canvas.DrawColor = GreyColor;
		else if(BTEC != None && BTEC.CustomTimer)
		{
			Canvas.DrawColor.R = BTEC.TimerRed;
			Canvas.DrawColor.G = BTEC.TimerGreen;
			Canvas.DrawColor.B = BTEC.TimerBlue;
		}
		else
...
{
	if(BTEC != None && BTEC.MuteAnnouncerAndHUDMessages)
